...
export default function CountrySelect({
  ...
  const filtered = options.filter(
    (o) => !query.trim() || o.toLowerCase().includes(query.toLowerCase())
  );
    ...
    <div className="relative w-full md:w-56 z-50" ref={containerRef}>
      ...
      {open && (
        <div className="absolute z-50 mt-2 w-full max-h-72 rounded-lg overflow-hidden border border-white/10 shadow-xl backdrop-blur-xl bg-gray-900/90">
          ...
          <ul
            role="listbox"
            className="max-h-60 overflow-y-auto custom-scrollbar text-sm"
          >
            {filtered.map((opt) => (
              <li key={opt}>
                <button
                  ...
                  className={`w-full text-left px-3 py-2 hover:bg-red-500/20 focus:bg-red-500/30 focus:outline-none transition-colors ${
                    opt === value
                      ? "text-white font-medium bg-red-500/30"
                      : "text-gray-300"
                  }`}
                  role="option"
