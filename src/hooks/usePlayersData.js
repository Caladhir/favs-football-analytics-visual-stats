...
export const usePlayersData = (options = {}) => {
  ...
  const fetchPlayers = useCallback(async () => {
    ...
    try {
      ...
      if (searchQuery.trim())
        base = base.ilike("full_name", `%${searchQuery}%`);

      const {
        data: playersData,
        error: playersErr,
        count,
      } = await base.order("full_name", { ascending: true }).limit(limit);
      if (playersErr)
        throw new Error(`Players fetch failed: ${playersErr.message}`);
      console.log(
        `📄 Paged fetch mode loaded ${playersData?.length || 0}/${count}`
      );
      ...
      const enrichedBase = playersData.map((pl) => ({
        ...pl,
        grouped_position: normalizeGroup(pl.position),
      }));
      let filtered = enrichedBase;
      if (league !== "all")
        filtered = filtered.filter(
          (p) => p.teams?.country?.toLowerCase() === league.toLowerCase()
        );
      if (position !== "all")
        filtered = filtered.filter((p) => p.grouped_position === position);

      // Supplemental fetch: ensure top stat leaders appear even if outside alphabetical slice
      const supplemental = [];
      if (
        rankingMetric &&
        ["goals", "assists", "minutes", "rating"].includes(rankingMetric)
      ) {
        let orderCol =
          rankingMetric === "minutes"
            ? "total_minutes"
            : rankingMetric === "goals"
            ? "total_goals"
            : "total_assists";
        ...
        if (!topErr && topRows) {
          ...
          const topFiltered = topRows.filter((r) => {
            ...
            if (
              league !== "all" &&
              pl.teams?.country?.toLowerCase() !== league.toLowerCase()
            )
              return false;
            if (position !== "all") {
          ...
          const missingIds = topFiltered
            .map((r) => r.id)
            .filter((id) => !filtered.some((p) => p.id === id));
          if (missingIds.length) {
            ...
            if (!extraErr && extraPlayers) {
              const extraFiltered = extraPlayers.filter((pl) => {
                ...
                if (
                  position !== "all" &&
                  normalizeGroup(pl.position) !== position
                )
                  return false;
                return true;
              });
              supplemental.push(
                ...extraFiltered.map((pl) => ({
                  ...pl,
                  grouped_position: normalizeGroup(pl.position),
                }))
              );
              console.log(
                `➕ Supplemental added ${extraFiltered.length}/${extraPlayers.length} (${rankingMetric}) after filters`
              );
            }
          }
        }
      ...
      if (includeStats && filtered.length) {
        ...
        for (let i = 0; i < playerIds.length; i += batchSize) {
          let q = supabase
            .from("player_stats")
            .select(statsCols)
            .in("player_id", playerIds.slice(i, i + batchSize))
            .order("created_at", { ascending: false });
          if (statsFromDate)
            q = q.gte("created_at", statsFromDate.toISOString());
          const { data, error } = await q;
          if (error) {
            console.warn("Stats batch failed", error.message);
            continue;
          }
          statsData.push(...(data || []));
        }
      ...
      playersWithStats.sort((a, b) => {
        ...
        switch (sortBy) {
          case "rating":
            return (sb.rating || 0) - (sa.rating || 0) || tie(a, b);
          case "goals":
            return (
              (sb.goals || 0) - (sa.goals || 0) ||
              (sb.assists || 0) - (sa.assists || 0) ||
              (sb.minutes || 0) - (sa.minutes || 0) ||
              tie(a, b)
            );
          case "assists":
            return (
          ...
          case "goalsPer90":
            return (
              (sb.goalsPer90 || 0) - (sa.goalsPer90 || 0) ||
              (sb.minutes || 0) - (sa.minutes || 0) ||
              tie(a, b)
            );
          case "assistsPer90":
            return (
              (sb.assistsPer90 || 0) - (sa.assistsPer90 || 0) ||
              (sb.minutes || 0) - (sa.minutes || 0) ||
              tie(a, b)
            );
          case "minutes":
            return (sb.minutes || 0) - (sa.minutes || 0) || tie(a, b);
          case "shotAccuracy":
            return (sb.shotAccuracy || 0) - (sa.shotAccuracy || 0) || tie(a, b);
          case "team":
            return (
  ...
  const fetchSeasonLeaders = useCallback(async () => {
    try {
      ...
      const baseSelect =
        "id, full_name, total_goals, total_assists, total_matches, total_minutes";
      ...
      if (goalsData)
        setSeasonTopScorers(goalsData.filter((p) => (p.total_goals || 0) > 0));
      if (assistsData)
        setSeasonTopAssists(
          assistsData.filter((p) => (p.total_assists || 0) > 0)
        );
      ...
      if (ratingRows && ratingRows.length) {
        ...
        const ratedList = Object.entries(agg)
          ...
          .map(([player_id, v]) => ({
            player_id,
            avg_rating: +(v.sum / v.count).toFixed(1),
            minutes: v.minutes,
          }));
        ...
        if (topRatingIds.length) {
          ...
          const nameMap = Object.fromEntries(
            (namesData || []).map((p) => [p.id, p.full_name])
          );
          // Merge with totals for possible team/minutes if present
          const totalsMap = {};
          [...(goalsData || []), ...(assistsData || [])].forEach((p) => {
            totalsMap[p.id] = p;
          });
  ...
  const fetchMetadata = useCallback(async () => {
    try {
      ...
      if (positionsData) {
        ...
        const ordered = ["GK", "DEF", "MID", "FOR"].filter((g) =>
          groupsSet.has(g)
        );
